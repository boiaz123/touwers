...
; ---- Touwers dark/gold theme (matches in-game menu palette) ----
!define MUI_BGCOLOR "1A0F0A"
!define MUI_TEXTCOLOR "D4AF37"
!define MUI_LICENSEPAGE_BGCOLOR "1A0F0A"
; The rest of the standard wizard pages (License/Directory/StartMenu/InstFiles)
; can only have their overall canvas recolored safely - the instructional
; labels on them keep the system default (black) text, which stays readable
...
; Touwers custom welcome page
Var TouwersWelcomeDlg
...
; ---- Touwers theming helper -------------------------------------------
; Recolors the current page's inner dialog canvas to the game's dark/gold
; palette. Safe on every MUI page: it just finds the page-local "#32770"
; child dialog that already exists and sets its background/text brush,
; it never touches shared wizard chrome (Back/Next/Cancel).
!insertmacro MUI_PAGE_INIT
!insertmacro MUI_PAGE_FUNCTION_FULLWINDOW
Function TouwersThemePageCanvas
  ; Best-effort: on this Windows build the stock wizard page template paints
  ; its background via a UxTheme texture that ignores the classic
  ; WM_CTLCOLORDLG brush SetCtlColors registers here, so the page margin
  ; stays the system default. Left in place since it's harmless and is
...
; Replaces the stock MUI welcome dialog with a full-bleed piece of game
; art and a single centered INSTALL button (instead of the small Next
; button in the corner). Clicking it sends the wizard the same command
; the real Next button sends, so the rest of the page flow is untouched.
Function TouwersWelcomeShow
  ${IfThen} $PassiveMode = 1 ${|} Abort ${|}

  InitPluginsDir
  File "/oname=$PLUGINSDIR\touwers-welcome.bmp" "${TOUWERS_INSTALLER_DIR}\nsis-welcome.bmp"

  nsDialogs::Create 1044
  Pop $TouwersWelcomeDlg
  ...
  ; Restore Back/Next visibility for the rest of the wizard
  GetDlgItem $0 $HWNDPARENT 1
  ShowWindow $0 ${SW_NORMAL}
  GetDlgItem $0 $HWNDPARENT 3
  ShowWindow $0 ${SW_NORMAL}

  ${NSD_FreeImage} $TouwersWelcomeImageHandle
FunctionEnd

Function TouwersWelcomeInstallClick
  SendMessage $HWNDPARENT ${WM_COMMAND} 1 0
FunctionEnd

; Installer pages, must be ordered as they appear
; 1. Welcome Page
Page custom TouwersWelcomeShow
...
Function PageReinstall
  ...
  ${Else}
    ...
    Pop $R4
    ${IfThen} $(^RTL) = 1 ${|} nsDialogs::SetRTL $(^RTL) ${|}
    Call TouwersThemePageCanvas

    ${NSD_CreateLabel} 0 0 100% 24u $R1
    Pop $R1
    SetCtlColors $R1 "${TOUWERS_PAGE_TEXTCOLOR}" "${TOUWERS_PAGE_BGCOLOR}"

    ${NSD_CreateRadioButton} 30u 50u -30u 8u $R2
    Pop $R2
    SetCtlColors $R2 "${TOUWERS_PAGE_TEXTCOLOR}" "${TOUWERS_PAGE_BGCOLOR}"
    ${NSD_OnClick} $R2 PageReinstallUpdateSelection

    ${NSD_CreateRadioButton} 30u 70u -30u 8u $R3
    Pop $R3
    SetCtlColors $R3 "${TOUWERS_PAGE_TEXTCOLOR}" "${TOUWERS_PAGE_BGCOLOR}"
