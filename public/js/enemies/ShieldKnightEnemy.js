export class ShieldKnightEnemy {
    constructor(path, health = 180, speed = 35) {
        ...
        this.sizeMultiplier = 1.05; // Slightly taller than knight, shorter than beefy
        
        console.log('ShieldKnightEnemy: Created at position', this.x, this.y, 'with path length', path ? path.length : 0);
    }
    
    getRandomTunicColor() {
        // Shield knights get sophisticated metallic armor colors
        const tunicColors = [
            '#4A5568', // Steel Blue
    ...
    updatePath(newPath) {
        if (!newPath || newPath.length === 0) {
            console.warn('ShieldKnightEnemy: Received invalid path');
            return;
        }
        ...
        console.log('ShieldKnightEnemy: Path updated, now at index', this.currentPathIndex, 'position', this.x, this.y);
    }
    ...
    update(deltaTime) {
        ...
        if (this.currentPathIndex >= this.path.length - 1) {
            ...
            console.log('ShieldKnightEnemy: Reached end of path');
            return;
        }
        ...
        if (!target) {
            this.reachedEnd = true;
            console.log('ShieldKnightEnemy: No target waypoint, reached end');
            return;
        }
