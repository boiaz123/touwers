...
export class Campaign1 extends CampaignBase {
    ...
    // so the level-select backdrop matches actual in-level trees instead of drawing its
    // own separate, drifted-out-of-sync copy. `seed` picked explicitly as `variant`
    // bypasses TerrainRenderer's own gridX+gridY hashing, since this backdrop already
    // computed its own seed per tree position (see the `renderables.push` call site).
    _drawTreeAtSeed(ctx, x, y, size, seed) {
        this._levelBase.renderTree(ctx, x, y, size, 0, 0, seed);
    }
