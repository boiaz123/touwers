...
export class BasicTower extends Tower {
    ...
    drawEnvironmentBack(ctx, gridSize) {
        ...
        // Back trees: negative y = higher on screen = further from viewer
        const backTrees = [
        ...
        if (ctx.level) {
            ...
            // Back bushes (negative y)
            const backBushVegSize = gridSize * 0.28;
            ctx.level.renderVegetation(ctx, this.x + gridSize * 0.12,  this.y - gridSize * 0.10, backBushVegSize, 0, 0, 5);
            ctx.level.renderVegetation(ctx, this.x - gridSize * 0.18,  this.y - gridSize * 0.18, backBushVegSize, 0, 0, 6);
        } else {
            // Fallback: forest-only back trees
            const fallbackBackTrees = [
            ...
            fallbackBackTrees.forEach(tree => {
        ...
        // Back rock (negative y)
        const backRock = { x: gridSize * 0.28, y: -gridSize * 0.24, size: 0.25 };
        ctx.fillStyle = '#807f80';
        ctx.beginPath();
        ctx.arc(this.x + backRock.x, this.y + backRock.y, 2 * backRock.size, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#696969';
        ctx.lineWidth = 1;
        ctx.stroke();

        // Back grass patch (forest only)
        if (!ctx.level || ctx.level.getCampaign() === 'forest') {
            ctx.strokeStyle = '#2e8b2e';
            ctx.lineWidth = 1;
            const gx = this.x + gridSize * 0.08;
            const gy = this.y - gridSize * 0.12;
            for (let i = 0; i < 4; i++) {
    ...
    drawEnvironmentFront(ctx, gridSize) {
        // Front trees: positive y = lower on screen = closer to viewer
        const frontTrees = [
        ...
        if (ctx.level) {
            ...
            // Front bush (positive y)
            ctx.level.renderVegetation(ctx, this.x - gridSize * 0.14, this.y + gridSize * 0.16, gridSize * 0.28, 0, 0, 4);
        } else {
            // Fallback: forest-only front trees
            const fallbackFrontTrees = [
            ...
            fallbackFrontTrees.forEach(tree => {
                const treeX = this.x + tree.x;
                const treeY = this.y + tree.y;
                const scale = tree.size;
                ctx.fillStyle = 'rgba(0, 0, 0, 0.18)';
            ...
            // Front fallback bush
            const bushX = this.x - gridSize * 0.22;
            const bushY = this.y + gridSize * 0.22;
            const scale = 0.32;
            ctx.fillStyle = '#1f6f1f';
            ctx.beginPath();
            ctx.arc(bushX, bushY, 3 * scale, 0, Math.PI * 2);
        ...
        // Front rocks (positive y)
        const frontRocks = [
            { x: -gridSize * 0.3,  y: gridSize * 0.28, size: 0.22 },
            { x:  gridSize * 0.25, y: gridSize * 0.18, size: 0.15 }
        ];
        frontRocks.forEach(rock => {
            ctx.fillStyle = '#807f80';
            ctx.beginPath();
            ctx.arc(this.x + rock.x, this.y + rock.y, 2 * rock.size, 0, Math.PI * 2);
            ctx.fill();
            ctx.strokeStyle = '#696969';
            ctx.lineWidth = 1;
            ctx.stroke();
        ...
        // Front grass patch (forest only)
        if (!ctx.level || ctx.level.getCampaign() === 'forest') {
            ctx.strokeStyle = '#2e8b2e';
            ctx.lineWidth = 1;
            const gx = this.x - gridSize * 0.12;
            const gy = this.y + gridSize * 0.18;
            for (let i = 0; i < 4; i++) {
                const angle = -Math.PI/2 + (i-1.5)*0.2;
                ctx.beginPath();
                ctx.moveTo(gx, gy);
