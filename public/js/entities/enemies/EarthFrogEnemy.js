...
export class EarthFrogEnemy extends BaseEnemy {
    ...
    static _getColorTable() {
        if (!EarthFrogEnemy._colorTable) {
            const bases = ['rgba(139, 111, 71, ', 'rgba(184, 134, 11, ', 'rgba(160, 82, 45, '];
            EarthFrogEnemy._colorTable = bases.map(b =>
                Array.from({ length: 101 }, (_, i) => b + (i / 100).toFixed(2) + ')')
            );
        }
        return EarthFrogEnemy._colorTable;
    }
    ...
    takeDamage(amount, armorPiercingPercent = 0, damageType = 'physical', followTarget = false) {
        // Only take full elemental damage if it's air damage (the weakness)
        // Magic (arcane/classless) damage also passes through, reduced by magic resistance
        if (damageType !== 'air' && damageType !== 'magic') {
    ...
    renderDynamicParts(ctx, baseSize) {
        ...
        // Chest plate (armor - earth colored)
        ...
        const colorTable = EarthFrogEnemy._getColorTable();
...
const upperLength = baseSize * 0.035;
    ...
    drawBattleMageHat(ctx, baseSize) {
        ...
        ctx.fillStyle = 'rgba(204, 136, 0, 0.9)';
        ctx.beginPath();
        ctx.arc(baseSize * 0.08, -baseSize * 1.44, baseSize * 0.04, 0, Math.PI * 2);
        ctx.fill();
    }
