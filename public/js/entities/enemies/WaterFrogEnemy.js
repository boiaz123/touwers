...
export class WaterFrogEnemy extends BaseEnemy {
    ...
    constructor(path, health_multiplier = 1.0, speed = null, armour = null, magicResistance = null) {
        const baseStats = WaterFrogEnemy.BASE_STATS;
        ...
        this.skinColor = '#4A90E2'; // Water blue
        this.elementalType = 'water';
        this.vulnerableTo = 'earth'; // Only takes earth damage
    ...
    static getCachedColor(baseColor, type) {
        ...
        if (!WaterFrogEnemy.colorCache.has(key)) {
            let color;
            if (type === 'lighten') {
                color = WaterFrogEnemy.lightenColorStatic(baseColor, 0.2);
            } else if (type === 'lighten_body') {
                color = WaterFrogEnemy.lightenColorStatic(baseColor, 0.4);
            } else if (type === 'darken') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.25);
            } else if (type === 'darken_body') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.35);
            } else if (type === 'darken_leg') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.05);
            } else if (type === 'darken_detail') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.3);
            } else if (type === 'darken_mouth') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.4);
            } else if (type === 'darken_eye') {
                color = WaterFrogEnemy.darkenColorStatic(baseColor, 0.15);
            } else if (type === 'lighten_foot') {
                color = WaterFrogEnemy.lightenColorStatic(baseColor, 0.15);
            }
            WaterFrogEnemy.colorCache.set(key, color);
        }
        return WaterFrogEnemy.colorCache.get(key);
    }
    ...
    render(ctx) {
        ...
        if (!this.cachedLightenColor) {
            this.cachedLightenColor = WaterFrogEnemy.getCachedColor(this.skinColor, 'lighten');
            this.cachedDarkenColor = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken');
            this.cachedDarken2Color = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_body');
        }
        ...
        bodyGradient.addColorStop(0, WaterFrogEnemy.getCachedColor(this.skinColor, 'lighten'));
        ...
        // Belly (lighter color)
        ctx.fillStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'lighten_body');
        ctx.beginPath();
        ...
        // Left eye socket
        ctx.fillStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_eye');
        ctx.beginPath();
        ...
        // Wide frog mouth
        ctx.strokeStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_mouth');
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ...
        // Nostril details
        ctx.fillStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_detail');
        ctx.beginPath();
    ...
    drawFrogBackLeg(ctx, hipX, hipY, baseSize, isRight) {
        ...
        ctx.strokeStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_leg');
        ctx.lineWidth = baseSize * 0.2;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        ctx.beginPath();
        ...
        ctx.fillStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'lighten_foot');
        ctx.beginPath();
        ctx.ellipse(footX, footY, baseSize * 0.16, baseSize * 0.18, calfAngle, 0, Math.PI * 2);
        ctx.fill();
        
        // Toe details - webbing
        ctx.strokeStyle = WaterFrogEnemy.getCachedColor(this.skinColor, 'darken_detail');
        ctx.lineWidth = 0.8;
        for (let i = -1; i <= 1; i++) {
    ...
    drawWizardHat(ctx, baseSize) {
        // Hat body - large pointy cone - Water colored
        const hatGradient = ctx.createLinearGradient(-baseSize * 0.35, -baseSize * 0.8, baseSize * 0.35, -baseSize * 1.5);
        hatGradient.addColorStop(0, '#4A90E2');
        hatGradient.addColorStop(0.6, '#003DA5');
        hatGradient.addColorStop(1, '#001A52');
    ...
    lightenColor(color, factor) {
        return WaterFrogEnemy.lightenColorStatic(color, factor);
    }
    
    darkenColor(color, factor) {
        return WaterFrogEnemy.darkenColorStatic(color, factor);
    }
}
