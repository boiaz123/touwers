...
export class AirFrogEnemy extends BaseEnemy {
    ...
    constructor(path, health_multiplier = 1.0, speed = null, armour = null, magicResistance = null) {
        const baseStats = AirFrogEnemy.BASE_STATS;
        ...
        this.skinColor = '#e8e8f8'; // Air grey-white
    ...
    takeDamage(amount, armorPiercingPercent = 0, damageType = 'physical', followTarget = false) {
        // Only take full elemental damage if it's fire damage (the weakness)
    ...
    static getCachedColor(baseColor, type) {
        ...
        if (!AirFrogEnemy.colorCache.has(key)) {
            let color;
            if (type === 'lighten') {
                color = AirFrogEnemy.lightenColorStatic(baseColor, 0.2);
            } else if (type === 'lighten_body') {
                color = AirFrogEnemy.lightenColorStatic(baseColor, 0.4);
            } else if (type === 'darken') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.25);
            } else if (type === 'darken_body') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.35);
            } else if (type === 'darken_leg') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.05);
            } else if (type === 'darken_detail') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.3);
            } else if (type === 'darken_mouth') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.4);
            } else if (type === 'darken_eye') {
                color = AirFrogEnemy.darkenColorStatic(baseColor, 0.15);
            } else if (type === 'lighten_foot') {
                color = AirFrogEnemy.lightenColorStatic(baseColor, 0.15);
            }
            AirFrogEnemy.colorCache.set(key, color);
        }
        return AirFrogEnemy.colorCache.get(key);
    }
    ...
    render(ctx) {
        ...
        // --- MAIN BODY/CHEST ---
        const bodyGrad = ctx.createLinearGradient(-baseSize * 0.4, -baseSize * 0.15, baseSize * 0.4, baseSize * 0.15);
        bodyGrad.addColorStop(0, '#f0f0f8');
        bodyGrad.addColorStop(0.5, '#e0e0f0');
        ...
        // Chest plate (armor - white/silver)
...
const upperLength = baseSize * 0.035;
    ...
    drawBattleMageHat(ctx, baseSize) {
        ...
        hatGradient.addColorStop(0, '#e0e0f8');
        hatGradient.addColorStop(0.5, '#d0d0f0');
        hatGradient.addColorStop(1, '#b0b0e0');
    ...
    lightenColor(color, factor) {
        return AirFrogEnemy.lightenColorStatic(color, factor);
    }
    
    darkenColor(color, factor) {
        return AirFrogEnemy.darkenColorStatic(color, factor);
    }
}
