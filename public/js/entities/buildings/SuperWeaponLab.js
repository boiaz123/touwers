...
export class SuperWeaponLab extends Building {
    ...
    /** All decorative offsets below are expressed as fractions of `size` (annotated per group
     *  against a size=128 reference, i.e. cellSize=32 at the default 1x resolution scale - see
     *  getCellSize()'s fallback formula) rather than fixed pixel values. The structural shapes
     *  (octagon width, spire taper, crystal height) already scaled with `size`; the decoration
     *  positioned on top of them (corner stones, windows, rune bands, floating runes, crystal
     *  size) previously didn't, so at any size other than the ~128 reference they'd drift out
     *  of proportion - windows creeping off the spire's actual taper, corner stones sitting at
     *  the wrong height relative to the base, etc. This is what "depth and perspective look
     *  correct at any size" means here: it's a scale-correctness bug, not a draw-order one -
     *  SuperWeaponLab has no circles/rings of the kind that caused CombinationTower's
     *  containment bugs, so there's no z-order issue to fix, just proportionality. */
    renderStoneBase(ctx, size) {
        const baseWidth = size * 0.7;
        const baseHeight = size * 0.25;
        const baseY = this.y + size * 0.156; // was "+20" @ size=128
        ...
        for (let i = 0; i < sides; i++) {
            ...
            const y = baseY + Math.sin(angle) * baseWidth/4;
        ...
        ctx.lineWidth = 1;
        for (let i = 0; i < 3; i++) {
            const y = this.y + size * 0.078 + i * size * 0.0625; // was "+10 + i*8" @ size=128
            ctx.beginPath();
            ctx.moveTo(this.x - baseWidth/2 + size * 0.078, y);
            ctx.lineTo(this.x + baseWidth/2 - size * 0.078, y);
            ctx.stroke();
        }

        // Corner stones - light gradient added for a touch more depth than the old flat fill
        const cornerHalf = size * 0.031; // was fillRect(...,8,8) i.e. half=4 @ size=128
        const cornerPositions = [
            { x: -baseWidth/2 + size * 0.0625, y: size * 0.117 },
            { x: baseWidth/2 - size * 0.0625, y: size * 0.117 },
            { x: -baseWidth/2 + size * 0.094, y: size * 0.195 },
            { x: baseWidth/2 - size * 0.094, y: size * 0.195 }
        ];

        cornerPositions.forEach(corner => {
            const cx = this.x + corner.x, cy = this.y + corner.y;
            const stoneGrad = ctx.createLinearGradient(cx - cornerHalf, cy - cornerHalf, cx + cornerHalf, cy + cornerHalf);
            stoneGrad.addColorStop(0, '#D1D5DB');
            stoneGrad.addColorStop(1, '#9CA3AF');
            ctx.fillStyle = stoneGrad;
            ctx.fillRect(cx - cornerHalf, cy - cornerHalf, cornerHalf * 2, cornerHalf * 2);
            ctx.strokeStyle = '#374151';
            ctx.lineWidth = 1;
            ctx.strokeRect(cx - cornerHalf, cy - cornerHalf, cornerHalf * 2, cornerHalf * 2);
        });
    }
    ...
    renderWoodenSupports(ctx, size) {
        ...
        const beamPositions = [
            { x: -0.195, y: 0, angle: -0.3 },
            { x: 0.195, y: 0, angle: 0.3 },
            { x: -0.141, y: 0.039, angle: -0.15 },
            { x: 0.141, y: 0.039, angle: 0.15 }
        ];
        const beamHalfWidth = size * 0.023;
        const beamLen = size * 0.234;

        beamPositions.forEach(beam => {
            ctx.save();
            ctx.translate(this.x + beam.x * size, this.y + beam.y * size);
            ctx.rotate(beam.angle);

            // Wood beam
            const woodGradient = ctx.createLinearGradient(-beamHalfWidth, -beamLen, beamHalfWidth, 0);
            woodGradient.addColorStop(0, '#92400E');
            woodGradient.addColorStop(0.5, '#78350F');
            woodGradient.addColorStop(1, '#451A03');

            ctx.fillStyle = woodGradient;
            ctx.fillRect(-beamHalfWidth, -beamLen, beamHalfWidth * 2, beamLen + size * 0.039);
            ctx.strokeStyle = '#2c1002';
            ctx.lineWidth = 1;
            ctx.strokeRect(-beamHalfWidth, -beamLen, beamHalfWidth * 2, beamLen + size * 0.039);

            // Wood grain
            ctx.strokeStyle = '#451A03';
            ctx.lineWidth = 1;
            for (let i = 0; i < 4; i++) {
                const y = -beamLen + size * 0.023 + i * size * 0.055;
                ctx.beginPath();
                ctx.moveTo(-beamHalfWidth * 0.7, y);
                ctx.lineTo(beamHalfWidth * 0.7, y + size * 0.016);
                ctx.stroke();
            }
        ...
        // Decorative iron bands - drawn with ctx.arc() (properly supports start/end angles
        // through CanvasGraphicsShim) rather than ctx.ellipse(), which silently ignores them.
        ctx.strokeStyle = '#374151';
        ctx.lineWidth = Math.max(1.5, size * 0.023);
        [-0.156, -0.039, 0.078].forEach(fy => {
            ctx.beginPath();
            ctx.arc(this.x, this.y + fy * size, size * 0.156, Math.PI * 0.8, Math.PI * 2.2);
            ctx.stroke();
        });
    }
    ...
    renderMainSpire(ctx, size) {
        ...
        const spireBaseY = this.y - size * 0.078; // was "-10" @ size=128
        const spireTopY = this.y - spireHeight + size * 0.156; // was "-spireHeight+20" @ size=128
        ...
        // Rim-light along both tapered edges - a thin bright stroke that only lights the
        // side facing the implied light source, giving the spire's silhouette a clearer
        // sense of depth than the flat gradient fill alone provided.
        ...
        // Stone block lines - sampled from the same width-interpolation formula as the spire
        // polygon above, so they stay correctly inset at every height regardless of size.
        ctx.strokeStyle = '#1F2937';
        ctx.lineWidth = 1;
        for (let i = 1; i < 8; i++) {
            const t = i / 8;
            const y = spireBaseY - (spireBaseY - spireTopY) * t;
        ...
        const windowPositions = [
            { t: -0.273, size: size * 0.047 },
            { t: -0.430, size: size * 0.039 },
        ...
        windowPositions.forEach(win => {
            ...
            ctx.beginPath();
            ctx.arc(this.x, wy, win.size + size * 0.016, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.arc(this.x, wy, win.size, 0, Math.PI * 2);
            ctx.fill();

            // Bright core spark - a small crisp highlight so the window reads as genuinely
            // lit rather than just a soft blob.
            ctx.fillStyle = `rgba(255, 255, 255, ${0.4 + this.runeGlow * 0.4})`;
            ctx.beginPath();
            ctx.arc(this.x, wy, win.size * 0.3, 0, Math.PI * 2);
            ctx.fill();
        });

        // Carved rune bands
        const runeBandT = [-0.195, -0.391, -0.547];
        runeBandT.forEach((t, index) => {
            const by = this.y + t * size;
            const bandWidth = spireBaseWidth - size * 0.039 - index * size * 0.023;

            ctx.strokeStyle = `rgba(139, 92, 246, ${this.runeGlow})`;
            ctx.lineWidth = Math.max(1, size * 0.016);
            ctx.beginPath();
            ctx.moveTo(this.x - bandWidth/2 + size * 0.039, by);
            ctx.lineTo(this.x + bandWidth/2 - size * 0.039, by);
            ctx.stroke();

            // Rune symbols on band
            ctx.fillStyle = `rgba(167, 139, 250, ${this.runeGlow})`;
            ctx.font = `${Math.round(size * 0.0625)}px serif`;
            ctx.textAlign = 'center';
            const symbols = ['◇', '✧', '❋'];
            ctx.fillText(symbols[index], this.x, by + size * 0.023);
        });
    }
    ...
    renderFloatingRunes(ctx, size) {
        ...
        this.floatingRunes.forEach((rune, index) => {
            ...
            const x = this.x + Math.cos(currentAngle) * orbitRadius;
            ...
            ctx.font = `bold ${Math.round(size * 0.078)}px serif`;
            ctx.textAlign = 'center';
    ...
    renderCrystalTop(ctx, size) {
        ...
        const crystalSize = size * 0.094; // was fixed 12 @ size=128 - previously didn't scale at all
        const platformR = size * 0.0625, platformFlat = size * 0.031, platformY = size * 0.117;
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const x = this.x + Math.cos(angle) * platformR;
            const y = crystalY + platformY + Math.sin(angle) * platformFlat;
            if (i === 0) ctx.moveTo(x, y);
            else ctx.lineTo(x, y);
        }
        ctx.closePath();
